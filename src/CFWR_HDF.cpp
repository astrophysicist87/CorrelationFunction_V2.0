...
int CorrelationFunction::Set_giant_chunked_HDF_array()
{
	...
	try
    {
		...
		hsize_t dims[RANK] = {giant_array_size};	// == FO_length * local_small_array_size
		dataspace = new H5::DataSpace (RANK, dims);
...
//int CorrelationFunction::Get_chunk(int isurf, double local_small_array[][eta_s_npts * qnpts * qnpts * qnpts * qnpts * 2])		//RANKV2 version
...
int CorrelationFunction::Reset_HDF()
{
	...
	try
	{
		...
		hsize_t dimsm[RANKV2] = {1, local_small_array_size};
		memspace = new H5::DataSpace (RANKV2, dimsm, NULL);  
	}
...
int CorrelationFunction::Initialize_resonance_HDF_array()
{
	...
	try
    {
		...
		DSetCreatPropList cparms;
		hsize_t chunk_dims[resonance_array_rank] = {chunk_size};
		cparms.setChunk( resonance_array_rank, chunk_dims );

		hsize_t dims[resonance_array_rank] = {Nparticle * chunk_size};
		resonance_dataspace = new H5::DataSpace (resonance_array_rank, dims);
		...
		hsize_t dimsm[resonance_array_rank] = {chunk_size};
		hsize_t offset[resonance_array_rank] = {0};

		resonance_memspace = new H5::DataSpace (resonance_array_rank, dimsm, NULL);
		for (int ir = 0; ir < Nparticle; ++ir)
		{
			...
			{
				resonance_chunk[iidx] = dN_dypTdpTdphi_moments[ir][ipt][iphi][iqt][iqx][iqy][iqz][itrig];
				//ln_dN_dypTdpTdphi_moments[ir][ipt][iphi][iqt][iqx][iqy][iqz][itrig] = log(abs(temp)+1.e-100);
				//sign_of_dN_dypTdpTdphi_moments[ir][ipt][iphi][iqt][iqx][iqy][iqz][itrig] = sgn(temp);
				++iidx;
			}*/
...
int CorrelationFunction::Get_resonance_from_HDF_array(int local_pid, double ******* resonance_array_to_fill)
{
	...
	try
    {
		...
		hsize_t offset[resonance_array_rank] = {local_pid * chunk_size};
		hsize_t count[resonance_array_rank] = {chunk_size};				// == chunk_dims
		resonance_dataspace->selectHyperslab(H5S_SELECT_SET, count, offset);
		...
		for (int itrig = 0; itrig < ntrig; ++itrig)
		{
			...
			//ln_dN_dypTdpTdphi_moments[ipt][iphi][iqt][iqx][iqy][iqz][itrig] = log(abs(temp)+1.e-100);
			//sign_of_dN_dypTdpTdphi_moments[ipt][iphi][iqt][iqx][iqy][iqz][itrig] = sgn(temp);
			++iidx;
		}
