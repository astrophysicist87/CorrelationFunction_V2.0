...
double CorrelationFunction::form_cos_sign_z(int isurf, int ieta, int iqt, int iqx, int iqy, int iqz, double zfactor)
{
	// zfactor == +/- 1
	...
	return	( cosA0*cosA1*cosA2*cosA3
...
double CorrelationFunction::form_sin_sign_z(int isurf, int ieta, int iqt, int iqx, int iqy, int iqz, double zfactor)
{
	// zfactor == +/- 1
	double cosA0 = osc0[isurf][ieta][iqt][0], cosA1 = osc1[isurf][iqx][0], cosA2 = osc2[isurf][iqy][0], cosA3 = osc3[isurf][ieta][iqz][0];
	double sinA0 = osc0[isurf][ieta][iqt][1], sinA1 = osc1[isurf][iqx][1], sinA2 = osc2[isurf][iqy][1], sinA3 = osc3[isurf][ieta][iqz][1];
	return ( cosA1*cosA2*cosA3*sinA0
...
{
	//double ***** local_temp_moments;
	//double ******* temp_moments_array;
	//Setup_temp_arrays(local_temp_moments, temp_moments_array);
...
//						double qx = qx_pts[iqx];
//						double qy = qy_pts[iqy];
...
//							local_temp_moments[iqt][iqx][iqy][iqz][0] += cos(arg)*S_p_withweight;
//							local_temp_moments[iqt][iqx][iqy][iqz][1] += sin(arg)*S_p_withweight;
//							local_temp_moments[iqt][iqx][iqy][iqz][0] += S_p_withweight;
//							local_temp_moments[iqt][iqx][iqy][iqz][1] += 0.0*S_p_withweight;
							zf *= -1.0;
							local_temp_moments[iqt][iqx][iqy][iqz][0] += form_cos_sign_z(isurf, ieta, iqt, iqx, iqy, iqz, zf) * S_p_withweight;
							local_temp_moments[iqt][iqx][iqy][iqz][1] += form_sin_sign_z(isurf, ieta, iqt, iqx, iqy, iqz, zf) * S_p_withweight;
			...
			for (int iqz = 0; iqz < qnpts; ++iqz)
			for (int itrig = 0; itrig < 2; ++itrig)
				temp_moments_array[ipt][iphi][iqt][iqx][iqy][iqz][itrig] = local_temp_moments[iqt][iqx][iqy][iqz][itrig];
		}
tempsw.toc();
*global_out_stream_ptr << "...finished pT = " << pT << " in " << tempsw.takeTime() << " seconds." << endl;
if (1) exit(1);
	}
	...
	for (int itrig = 0; itrig < 2; ++itrig)
	{
		double temp = temp_moments_array[ipt][iphi][iqt][iqx][iqy][iqz][itrig];
	...
	// clean up
	//Teardown_temp_arrays(local_temp_moments, temp_moments_array);
